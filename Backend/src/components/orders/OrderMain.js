...
const OrderMain = () => {
  ...
  const getFilterStatus = () => {
    ...
    } else if (selectedStatus === "Chờ giao hàng") {
      return searchOrders?.filter(
  ...
  const getSortOrder = () => {
    ...
    } else if (selectedSort === "Đơn đặt mới nhất") {
      return filterStatus?.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );
    } else if (selectedSort === "Đơn đặt cũ nhất") {
      return filterStatus?.sort(
  ...
  return (
    <section className="content-main">
      <div className="content-header">
        <h2 className="content-title">Quản lí đơn hàng</h2>
      </div>
